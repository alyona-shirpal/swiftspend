...
export const OAuthButtons: React.FC<OAuthButtonsProps> = ({ onError, onSuccess }) => {
  const handleOAuthLogin = async (provider: 'google' | 'apple') => {
    try {
      ...
      // Note: onSuccess isn't strictly needed here because signInWithOAuth will redirect the page away
      onSuccess?.();
    } catch (err: unknown) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:gap-4">
      <button 
        ...
        className="flex items-center justify-center gap-2 sm:gap-3 py-3 sm:py-4 border border-outline-variant/30 rounded-lg font-body text-sm font-medium hover:bg-surface-container-low transition-colors"
      >
        ...
        <span className="font-label text-[10px] font-bold uppercase tracking-widest text-primary">Google</span>
      </button>
      ...
      >
        ...
        <span className="font-label text-[10px] font-bold uppercase tracking-widest text-primary">Apple</span>
      </button>
    </div>
  );
};
